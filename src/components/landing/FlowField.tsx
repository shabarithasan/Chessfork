...
type ColorTheme = "aurora" | "ember" | "ocean" | "chess";
...
export function FlowField({
  ...
  useEffect(() => {
    ...
    const pickBg = () => (isLightRef.current ? "246, 246, 244" : cfg.bg);
    const pickTrailAlpha = () => (isLightRef.current ? 0.14 : cfg.trailAlpha);
    ...
    const render = () => {
      ...
      ctx.fillStyle = `rgba(${bg}, ${pickTrailAlpha()})`;
      ctx.fillRect(0, 0, width, height);
  ...
  const bgColor = isLight ? "246, 246, 244" : THEMES[theme].bg;
