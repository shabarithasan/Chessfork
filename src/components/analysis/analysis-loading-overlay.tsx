...
export function AnalysisLoadingExperience({
  ...
  return (
    <div
      className={cn(
        "overflow-hidden bg-[rgba(9,9,11,0.88)] ",
        presentation === "overlay" ? "fixed inset-0 z-50 overflow-y-auto" : "relative w-full rounded-xl border border-neutral-800",
      )}
    >
      ...
      >
        <div className="grid w-full min-w-0 gap-4 xl:grid-cols-[minmax(0,1.28fr)_25rem]">
          <div className="min-w-0 rounded-lg border border-neutral-800 bg-[linear-gradient(180deg,rgba(36,36,39,0.98),rgba(24,24,27,0.98))] p-3 shadow-[0_40px_110px_rgba(0,0,0,0.5)] sm:rounded-xl sm:p-4">
            <div className="relative">
              ...
              <div className="absolute left-4 top-4 z-30 flex flex-wrap gap-2">
                <span className="rounded-full border border-amber-400/20 bg-black/45 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-amber-400 ">
                  Engine scan
                </span>
                <span className="rounded-full border border-emerald-400/20 bg-black/45 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-emerald-400 ">
                  Real game preview
                </span>
              </div>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={item.label} className="rounded-lg border border-neutral-800 bg-black/18 px-4 py-3">
                    <p className="text-[0.65rem] font-semibold uppercase tracking-[0.22em] text-neutral-500">{item.label}</p>
                    <p className="mt-1 truncate text-sm font-semibold text-neutral-100">{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <aside className="min-w-0 rounded-lg border border-neutral-800 bg-[linear-gradient(180deg,rgba(20,20,23,0.98),rgba(12,12,14,0.98))] p-4 shadow-[0_40px_110px_rgba(0,0,0,0.5)] sm:rounded-xl sm:p-5">
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/30 p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="analysis-pulse-ring grid size-12 place-items-center rounded-2xl bg-amber-400/15 text-amber-400">
                    <Bot className="size-6" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.28em] text-amber-400">Analyzing game</p>
                    <p className="mt-1 text-sm text-neutral-400">{sourceLabels[source]}</p>
                  </div>
                </div>
                <span className="rounded-full border border-neutral-800 bg-neutral-800/30 px-3 py-1 text-xs font-semibold text-neutral-300">
                  {progress}%
                </span>
              </div>

              <p className="mt-5 text-3xl font-semibold leading-tight text-white sm:text-4xl">{activeStage.label}</p>
              <p className="mt-3 text-sm leading-7 text-neutral-300">{sourceCopy[source]}</p>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={item.label} className="rounded-[1rem] border border-neutral-800 bg-neutral-950/40 px-3 py-3">
                    <p className="text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-neutral-500">{item.label}</p>
              ...
              {loadingStages.map((stage, index) => {
                ...
                return (
                  <div
                    key={stage.label}
                    className={cn(
                      "rounded-lg border px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]",
                      active
                    ...
                    <div className="flex items-start gap-3">
                      <div
                        className={cn(
                          ...
                          completed
                            ? "border-emerald-400/25 bg-emerald-400/12 text-emerald-400"
                            : active
                              ? "border-amber-400/25 bg-amber-400/12 text-amber-400"
                              : "border-neutral-800 bg-neutral-800/30 text-neutral-500",
                        )}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <p className={cn("font-semibold", active || completed ? "text-white" : "text-neutral-400")}>{stage.label}</p>
                          <span className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-neutral-500">{stage.metric}</span>
                        </div>
                        <p className="mt-1 text-sm leading-6 text-neutral-500">{stage.copy}</p>
            ...
            <div className="mt-4 rounded-lg border border-emerald-400/10 bg-[linear-gradient(135deg,rgba(47,79,57,0.24),rgba(255,255,255,0.03))] p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-400">PV queue</p>
                <span className="rounded-full border border-neutral-800 bg-neutral-800/30 px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-neutral-400">
                  waiting for Stockfish
                </span>
              </div>
              <div className="mt-3 space-y-2">
                {["Best line", "Alternative line", "Refutation check"].map((line, index) => (
                  <div key={line} className="grid grid-cols-[auto_1fr] items-center gap-3 rounded-[1rem] border border-neutral-800 bg-black/14 px-3 py-2.5">
                    <span className="grid size-7 place-items-center rounded-full bg-white/[0.06] text-xs font-semibold text-neutral-300">{index + 1}</span>
                    <div className="min-w-0">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold text-white">{line}</p>
                        <p className="text-xs text-neutral-500">real PV only</p>
                      </div>
            ...
            <div className="mt-4 rounded-lg border border-neutral-800 bg-[linear-gradient(180deg,rgba(48,40,32,0.5),rgba(27,23,20,0.9))] p-4">
              <div className="flex items-start gap-3">
                <BrainCircuit className="mt-1 size-5 shrink-0 text-amber-400" />
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-400">Current brief</p>
...
export function AnalysisLoadingOverlay(props: Omit<AnalysisLoadingExperienceProps, "presentation">) {
  // Imports begin analysis in-place, so this is the loading experience users actually see
  // before the report route takes over. Keep it visually identical to /analysis/[id].
  void props;
