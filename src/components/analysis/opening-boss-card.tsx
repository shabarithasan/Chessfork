...
export function OpeningBossCard({
  ...
  return (
    <section
      id="opening-boss"
      className="rounded-[1.7rem] border border-fuchsia-200/15 bg-[linear-gradient(118deg,rgba(232,121,249,0.15),transparent_34%),linear-gradient(135deg,rgba(76,29,149,0.48),rgba(15,23,42,0.88)_46%,rgba(2,6,23,0.96))] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.22)]"
    >
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-2 rounded-full border border-fuchsia-200/25 bg-fuchsia-200/12 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-100">
          ...
          <p className="mt-5 text-sm font-semibold uppercase tracking-[0.2em] text-fuchsia-200">{boss.opening}</p>
          <h3 className="mt-2 text-3xl font-semibold tracking-tight text-white">{boss.bossName}</h3>
          <p className="mt-3 text-sm leading-7 text-slate-300">{boss.proof}</p>

          <div className="mt-5 rounded-[1rem] border border-white/10 bg-white/[0.04] p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Live HP</p>
        ...
        <div className="rounded-[1.25rem] border border-white/10 bg-slate-950/55 p-4">
          <div className="rounded-[1rem] border border-amber-300/15 bg-amber-300/8 p-4">
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-amber-200">
              <Swords className="size-4" />
              Best weapon
            </p>
            <p className="mt-2 text-sm leading-7 text-slate-100">{boss.weapon}</p>
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {boss.quests.map((quest) => (
              <button
                ...
                className="rounded-[1rem] border border-white/10 bg-white/[0.04] p-4 text-left transition hover:border-fuchsia-200/35 hover:bg-white/[0.07]"
              >
                ...
                <span className="mt-2 block text-xs leading-6 text-slate-400">{quest.target}</span>
              </button>
            ))}
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={copyBoss}
              className="inline-flex items-center gap-2 rounded-full bg-fuchsia-200 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-fuchsia-100"
            >
          ...
          {manualTextVisible ? (
            <textarea
              ...
              className="mt-4 min-h-36 w-full rounded-[1rem] border border-white/10 bg-slate-950/80 p-4 text-sm leading-6 text-slate-100 outline-none focus:border-fuchsia-200/60"
            />
          ) : null}
        </div>
      </div>
